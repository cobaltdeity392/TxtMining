...
combinedt<-read.transactions("combined_transactions.csv",
                           rm.duplicates = FALSE,
                           format = "basket",
                           sep = ",")
...
mediumrules = arules::apriori(mediumt,parameter = list(support=.02,
                                                       confidence=.5,
                                                       minlen=2))
combinedrules = arules::apriori(combinedt,parameter = list(support=.01,
                                                       confidence=.5,
                                                       minlen=2))
...
itemFrequencyPlot(combinedt, topN=20, type="absolute")
...
(summary(sortedmedium))
#########################
sortedcombo <- sort(combinedrules, by="confidence", decreasing=TRUE)
inspect(sortedcombo[1:15])
(summary(sortedcombo))

sortedcombo <- sort(combinedrules, by="support", decreasing=TRUE)
inspect(sortedcombo[1:15])
(summary(sortedcombo))

sortedcombo <- sort(combinedrules, by="lift", decreasing=TRUE)
inspect(sortedcombo[1:15])
(summary(sortedcombo))
#########################33
...
plot(redditrules, method="graph", control=list(type="items"))
plot(mediumrules, method="graph", control=list(type="items"))
...
reddit_viz_data <- rules_to_visNetwork(redditrules)

visNetwork(nodes = reddit_viz_data$nodes, edges = reddit_viz_data$edges) %>%
  visEdges(arrows = 'to') %>%
  visOptions(highlightNearest = TRUE, nodesIdSelection = TRUE) %>%
  visLayout(randomSeed = 123)


medium_viz_data <- rules_to_visNetwork(mediumrules)

visNetwork(nodes = medium_viz_data$nodes, edges = medium_viz_data$edges) %>%
  visEdges(arrows = 'to') %>%
  visOptions(highlightNearest = TRUE, nodesIdSelection = TRUE) %>%
  visLayout(randomSeed = 123)

combo_viz_data <- rules_to_visNetwork(combinedrules)

visNetwork(nodes = combo_viz_data$nodes, edges = combo_viz_data$edges) %>%
  visEdges(arrows = 'to') %>%
  visOptions(highlightNearest = TRUE, nodesIdSelection = TRUE) %>%
  visLayout(randomSeed = 123)
